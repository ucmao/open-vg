  ...
  <div class="min-h-screen bg-[#0a0a0f] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
    ...
    <div class="max-w-md w-full relative">
      ...
      <div class="text-center mb-8">
        ...
        <p class="mt-2 text-gray-500">Sign in to continue creating</p>
      </div>
      ...
      <div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-8">
        ...
        <div class="space-y-3">
          ...
          <!-- Placeholder for future social login buttons (Facebook, Apple, etc.) -->
          <!-- 
          <button
            @click="handleFacebookLogin"
            type="button"
            class="w-full py-3.5 bg-[#1877F2] hover:bg-[#166FE5] text-white font-semibold rounded-xl transition-all flex items-center justify-center space-x-3"
          >
          ...
          <button
            ...
            type="button"
            class="w-full py-3.5 bg-black hover:bg-gray-900 text-white font-semibold rounded-xl transition-all flex items-center justify-center space-x-3"
          >
...
const handleLogin = async () => {
  try {
    ...
    // Get redirect path from query parameter, default to home
...
const handleGoogleLogin = async () => {
  try {
    ...
    const redirectPath = route.query.redirect as string || '/'
    // Pass redirect path to backend so it can be included in callback
    const response = await api.get(`/api/auth/google/url?redirect=${encodeURIComponent(redirectPath)}`)
    if (response.success && response.data?.url) {
      // Open popup window for OAuth login
      const width = 500
      const height = 600
      ...
      // Check if popup is closed manually (user cancels) with COOP-safe error handling
