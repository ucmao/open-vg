        ...
        <div class="flex-1 overflow-y-auto px-4 pt-5 pb-4 sm:p-6">
          <div class="space-y-6">
            ...
            <div class="bg-gray-50 rounded-lg p-4 space-y-4 border border-gray-200">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700">
                  URL *
                  <span class="text-xs text-gray-500 font-normal ml-1">（，：1920x1080px）</span>
                </label>
            ...
            <div class="bg-gray-50 rounded-lg p-4 space-y-4 border border-gray-200">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Title（：、、， HTML Edit）</label>
                <ClientOnly>
                  <RichTextEditor
...
const save = async () => {
  ...
  try {
    ...
    if (response.success) {
      toast.success(props.slide ? 'successful' : 'successful')
      emit('saved')
    } else {
      toast.error(response.message || 'Savefailed')
    }
  } catch (error: any) {
    toast.error('Savefailed: ' + (error.message || ''))
  } finally {
    saving.value = false
  }
}
