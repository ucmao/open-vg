  ...
  <div class="p-6">
    <div class="mb-6 flex justify-between items-center">
      ...
      <div class="flex items-center gap-2">
        ...
        >
          {{ loadingEffectsTree ? $adminT('Loading...', '加载中...') : $adminT('Refresh', '刷新') }}
        </button>
        <button
    ...
    <div v-else-if="effectsTree.length > 0">
      ...
      <div class="flex items-center justify-start gap-4 mb-2 px-1 text-sm text-gray-500">
        <button type="button" @click="expandAllEffectsParents" class="hover:text-gray-700 transition-colors">{{ $adminT("Expand All", "展开全部") }}</button>
        ...
        <div class="group flex items-center justify-between mb-3 pb-3 border-b relative pr-32">
          <div class="flex-1 flex items-center gap-2 min-w-0">
            <button
              ...
              :aria-label="expandedEffectsParentIds[parent.id] !== false ? $adminT('Collapse', '收起') : $adminT('Expand', '展开')"
            >
              <ChevronDown
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              type="text"
              class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500"
              :placeholder="$adminT('VHS Effects', '例如: VHS Effects')"
            />
          </div>
          ...
          <div>
            ...
            />
            <p class="text-xs text-gray-500 mt-1"> {{ $adminT("Paths will be generated automatically by the system (structure: / /obs/I/II)", "路径将由系统自动生成 (结构: /effects/一级/二级)") }}</p>
          </div>
...
const loadEffectsTree = async () => {
  ...
  try {
    ...
    toast.error(adminT('Failed to load the effects category tree', '加载特效分类树失败'))
...
const batchDeleteEffects = async () => {
  ...
  const confirmed = await confirm({
    ...
    message: adminT('Delete {count} selected categories? This action cannot be undone.', '确定删除选中的 {count} 个分类吗？此操作不可撤销。', { count: selectedEffectsIds.value.length }),
    type: 'warning'
  })
  if (!confirmed) return
  const ids = [...selectedEffectsIds.value]
...
const saveEffectsConfig = async () => {
  ...
  try {
    ...
    if (editingEffectsConfig.value.id) {
      ...
      if (response.success) {
        toast.success(adminT('Effects category created', '特效分类创建成功'))
        closeEffectsModal()
        await loadEffectsTree()
      }
    }
