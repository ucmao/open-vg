        ...
        <div class="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-xl min-w-0 flex-1 basis-40">
          ...
          <div class="min-w-0">
            ...
            <div class="text-lg font-bold text-gray-900 flex items-center gap-2 flex-wrap">
              ...
              <NuxtLink to="/moderation/reports" class="text-red-800 hover:text-red-900 hover:underline"> {{ snapshot?.pending_reports_count ?? 0 }}</NuxtLink>
            </div>
          </div>
        </div>
        <div class="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-xl min-w-0 flex-1 basis-40">
          ...
          <div class="min-w-0">
            ...
            <div class="text-lg font-bold text-gray-900 flex items-center gap-1.5 flex-wrap">
              ...
              <span class="text-gray-300">·</span>
              <NuxtLink to="/users/works" class="hover:text-blue-600 hover:underline transition-colors cursor-pointer"> {{ snapshot?.total_works ?? '—' }}</NuxtLink>
            </div>
          </div>
        </div>
      </div>
...
const consumeSuccessRate = computed(() => {
  if (!period.value) return 0
  const attempt = period.value.current.consume_frequency || 0
  const ok = period.value.current.successful_works || 0
  if (attempt === 0) return 0
  return Math.min(100, (ok / attempt) * 100)
})
