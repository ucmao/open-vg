  ...
  <div class="max-w-7xl mx-auto">
    ...
    <div v-if="loading" class="flex items-center justify-center min-h-[400px]">
      <div class="text-center">
        <div class="w-10 h-10 border-3 border-gray-200 border-t-blue-600 rounded-full animate-spin mx-auto mb-4"></div>
        <p class="text-gray-500">{{ $adminT("Loading articles...", "正在加载文章...") }}</p>
      </div>
    </div>
    ...
    <template v-else>
      ...
      <div class="sticky top-0 z-10 bg-gray-50 -mx-4 px-4 py-4 border-b border-gray-200 mb-8">
        <div class="flex items-center justify-between">
          ...
          <div>
            <h1 class="text-xl font-semibold text-gray-900">{{ $adminT("Edit post", "编辑文章") }}</h1>
        ...
        <div class="flex items-center space-x-3">
          ...
          >
            {{ form.status === 'draft' ? $adminT('Draft', '草稿') : form.status === 'published' ? $adminT('Published', '已发布') : $adminT('Archived', '已归档') }}
          </span>
          <select
            v-model="form.status"
          ...
          >
            <option value="draft">{{ $adminT("Save as draft", "保存为草稿") }}</option>
            <option value="published">{{ $adminT("Release", "发布") }}</option>
            <option value="archived">{{ $adminT("Archive", "归档") }}</option>
          </select>
          <button
            type="button"
            @click="handleSubmit"
        ...
        <div class="flex gap-8">
          ...
          <div class="flex-1 min-w-0 space-y-8">
            ...
            <div>
              ...
              <div class="mt-2 flex items-center text-sm">
                ...
                <span v-if="!editingSlug" class="text-gray-600 font-medium">{{ form.slug || 'yourslug' }}</span>
                <input
                  v-else
                  v-model="form.slug"
                  type="text"
          ...
          <div class="w-[300px] flex-shrink-0 space-y-6">
            ...
            <div class="space-y-4">
              ...
              <!-- Sort Order -->
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1.5"> <span class="text-gray-400">{{ $adminT("The smaller the number, the more forward.", "(数字越小越靠前)") }}</span></label>
                <input
            ...
            <div class="bg-white border border-red-200 rounded-xl shadow-sm p-5">
              ...
              <button
                type="button"
                @click="confirmDelete"
                class="w-full px-4 py-2 text-sm text-red-600 border border-red-300 rounded-lg hover:bg-red-50 transition-colors"
              > {{ $adminT("Delete this post", "删除此文章") }} </button>
            </div>
          </div>
        </div>
      </form>
...
onMounted(() => {
  console.log('Edit Blog Post page mounted, id:', route.params.id)
  requireAuth()
  fetchPost()
...
const fetchPost = async () => {
  try {
    loading.value = true
    const response = await api.get(`/api/admin/blog/posts/${route.params.id}`)
    ...
    if (response.success) {
      ...
      form.is_featured = post.is_featured || false
      form.sort_order = post.sort_order || 0
      form.published_at = formatDateTimeLocal(post.published_at)
      
      // Load author information if available
      if (post.author_id) {
        form.author_id = post.author_id
        // If author info is included in post data, use it
        if (post.author) {
          selectedAuthor.value = post.author
        } else {
          // If author info is not included, fetch it by ID
          try {
            const authorResponse = await api.get(`/api/admin/users/${post.author_id}`)
            if (authorResponse.success && authorResponse.data) {
              selectedAuthor.value = {
                id: authorResponse.data.id,
                nickname: authorResponse.data.nickname,
                handle: authorResponse.data.handle,
      ...
      toast.error(response.message || adminT("Failed to load the post", "加载文章失败"))
    ...
    if (error.response?.status === 403) {
      ...
      toast.error(adminT("Article not found", "文章未找到"))
      router.push('/content/blog')
    } else {
      toast.error(adminT("Failed to load the post", "加载文章失败"))
    }
  } finally {
    loading.value = false
  }
...
const handleSubmit = async () => {
  try {
    ...
    const response = await api.put(`/api/admin/blog/posts/${route.params.id}`, {
      slug: form.slug.trim(),
    ...
    if (response.success) {
      toast.success(adminT("Articles updated!", "文章已更新！"))
      router.push('/content/blog')
    } else {
      toast.error(response.message || adminT("Could not update the post", "无法更新文章"))
    }
  } catch (error: any) {
    console.error('Failed to update post:', error)
    toast.error(error.message || adminT("Could not update the post", "无法更新文章"))
  } finally {
    saving.value = false
  }
}
...
const generateSEO = async () => {
  ...
  try {
    const response = await api.post(`/api/admin/blog/posts/${route.params.id}/generate-seo`)
    if (response.success) {
...
const confirmDelete = async () => {
  const confirmed = await confirm({
    ...
    message: adminT("Are you sure you want to delete this article? This operation cannot be revoked and all relevant data will be permanently deleted.", "您确定要删除此文章吗？此操作无法撤销，所有相关数据都将被永久删除。"),
  ...
  try {
    const response = await api.delete(`/api/admin/blog/posts/${route.params.id}`)
    
    if (response.success) {
      toast.success(adminT("Article deleted!", "文章已删除！"))
      router.push('/content/blog')
    } else {
      toast.error(response.message || adminT("Could not delete the post", "无法删除文章"))
    }
  } catch (error: any) {
    console.error('Failed to delete post:', error)
    toast.error(error.message || adminT("Could not delete the post", "无法删除文章"))
  }
}
</script>
