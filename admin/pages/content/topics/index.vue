<template>
  <div class="max-w-7xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
      <div>
        <h1 class="text-2xl font-bold text-gray-900">{{ $adminT("Thematic management", "专题管理") }}</h1>
        <p class="text-gray-500 mt-1"> {{ $adminT("Create and manage the sessions, templates and landing pages.", "创建和管理 Featured Topics、模板和落地页。") }}</p>
      </div>
      ...
      >
        <Plus class="w-5 h-5 mr-2" /> {{ $adminT("New topic", "新建专题") }} </NuxtLink>
    </div>

    <!-- Filters -->
    <div class="flex items-center gap-6 mb-4">
      <div class="flex items-center gap-2">
        <span class="text-sm text-gray-600">{{ $adminT("Type:", "类型：") }}</span>
        <select
          v-model="typeFilter"
        ...
        >
          ...
          <option value="topic">{{ $adminT("Topic", "专题") }}</option>
          <option value="magic">{{ $adminT("Model", "模型") }}</option>
        </select>
      </div>
      <div class="flex items-center gap-2">
        ...
        <table class="w-full min-w-max text-left border-collapse">
          <thead>
            <tr class="bg-gray-50 border-b border-gray-200">
              ...
              <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $adminT("Sort", "排序") }}</th>
              <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $adminT("Type", "类型") }}</th>
              <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider max-w-sm">{{ $adminT("Topic", "专题") }}</th>
              <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $adminT("Status", "状态") }}</th>
              <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $adminT("Browse", "浏览量") }}</th>
              <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $adminT("Created at", "创建时间") }}</th>
              <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right sticky right-0 z-10 bg-gray-50 shadow-[inset_4px_0_6px_-2px_rgba(0,0,0,0.08)]">{{ $adminT("Action", "操作") }}</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200">
            ...
            <tr v-else-if="topics.length === 0">
              <td colspan="9" class="px-6 py-20 text-center text-gray-500">
                ...
                <p>{{ $adminT("No topics found. Start creating the first one!", "未找到专题。开始创建第一个吧！") }}</p>
              </td>
            </tr>
            <tr 
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span v-if="topic.generation_model_id" class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-violet-100 text-violet-700">{{ $adminT("Model", "模型") }}</span>
                <span v-else class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-700">{{ $adminT("Topic", "专题") }}</span>
              </td>
              <td class="px-6 py-4 max-w-sm">
                <div class="flex items-center space-x-2">
                  ...
                  <div class="min-w-0 flex-1">
                    ...
                    <div v-if="topic.is_featured" class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-amber-100 text-amber-700 mt-1">{{ $adminT("Featured", "精选") }}</div>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
      ...
      <div v-if="totalTopics > 0" class="px-6 py-4 bg-gray-50 border-t flex items-center justify-between">
        <div class="flex items-center gap-4">
          <span class="text-sm text-gray-600">
            {{ $adminT('Showing {from}–{to} of {total} topics', '显示第 {from}–{to} 条，共 {total} 个专题', { from: (page - 1) * pageSize + 1, to: Math.min(page * pageSize, totalTopics), total: totalTopics }) }}
          </span>
          <div class="flex items-center gap-2">
        ...
        <div class="flex items-center gap-2">
          ...
          <div class="flex items-center gap-1">
            ...
            />
            <span class="text-sm text-gray-600">{{ $adminT('of {total}', '/ {total} 页', { total: Math.ceil(totalTopics / pageSize) }) }}</span>
          </div>
          <button
            @click="loadPage(page + 1)"
    ...
    >
      <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ $adminT("Bulk edit topics", "批量编辑专题") }}</h3>
        <p class="text-sm text-gray-600 mb-4">
           {{ selectedIds.length }} {{ $adminT("selected", "将更新选中的") }} <br /> {{ $adminT("topics will be updated.", "个专题。") }} </p>
        <div class="space-y-4">
          <div>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">{{ $adminT("Featured", "是否精选") }}</label>
            <select
...
useHead({
  title: adminT("Thematic management", "专题管理")
})
...
const stats = ref([
  { label: adminT("Total number of topics", "专题总数"), value: '0' },
  { label: adminT("Published", "已发布"), value: '0' },
  { label: adminT("Featured", "精选"), value: '0' },
  { label: adminT("Total browsing", "总浏览量"), value: '0' }
])
...
const handleBatchEdit = async () => {
  ...
  const confirmed = await confirm({
    ...
    message: adminT('Update the {n} selected topics?', '确定要更新选中的 {n} 个专题吗？', { n: selectedIds.value.length }),
    type: 'info'
  })
  ...
  try {
    ...
    toast.error(err.message || adminT("Bulk update failed", "批量更新失败"))
  } finally {
    saving.value = false
  }
}
...
const handleBatchDelete = async () => {
  const confirmed = await confirm({
    ...
    message: adminT('Delete the {n} selected topics? This cannot be undone.', '确定要删除选中的 {n} 个专题吗？此操作不可撤销！', { n: selectedIds.value.length }),
    type: 'danger',
  ...
  try {
    ...
    if (res.success) {
      toast.success(adminT("Batch delete complete", "批量删除成功"))
      clearSelection()
      fetchTopics()
    }
  } catch (err: any) {
    toast.error(err.message || adminT("Batch delete failed", "批量删除失败"))
  }
}
...
const updateStats = () => {
  ...
  stats.value = [
    { label: adminT("Total number of topics", "专题总数"), value: totalTopics.value.toString() },
    { label: adminT("Published", "已发布"), value: published.toString() },
    { label: adminT("Featured", "精选"), value: featured.toString() },
    { label: adminT("Total browsing", "总浏览量"), value: views.toLocaleString() }
  ]
}
...
const deleteTopic = async (topic: any) => {
  const confirmed = await confirm({
    title: adminT("Delete topic", "删除专题"),
    message: adminT('Delete topic "{title}"?', '确定要删除专题 "{title}" 吗？', { title: topic.title }),
    type: 'danger'
  })
  if (!confirmed) return
  ...
  try {
    ...
    if (res.success) {
      toast.success(adminT("Topic deleted", "专题已删除"))
      fetchTopics()
    }
  } catch (err) {
    toast.error(adminT("Failed to delete the topic", "删除专题失败"))
  }
}

const getStatusLabel = (status: string) => {
  switch (status) {
    case 'published': return adminT("Published", "已发布")
    case 'draft': return adminT("Draft", "草稿")
    case 'archived': return adminT("Archived", "已归档")
